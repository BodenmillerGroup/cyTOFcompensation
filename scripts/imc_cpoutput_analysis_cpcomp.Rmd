...
fn_sm = '../data/Figure_S5/paper_version_Spillover_Matrix_1_sm.csv'
...
cp_measurevars = c('IntegratedIntensity','MeanIntensity','MedianIntensity', 'MeanIntensityCompLS', 'MeanIntensityCompNNLS')
```
...
dat = tdat %>%
  ...
  data.matrix() %>%
  flowCore::flowFrame() %>%
  compCytof(sm, method='flow') %>%
  exprs() %>%
  as.data.table() %>%
  cbind(dplyr::select(tdat, one_of(metavars))) %>%
  melt.data.table(id.vars = metavars, value.name = 'MeanIntensityCompFlow', variable.name= 'metal_catalyst') %>%
  merge(dat_pannel[, .(metal_catalyst, channelnr)], by='metal_catalyst') %>%
  dplyr::select(-metal_catalyst) %>%
  merge(dat, by=c(metavars, 'channelnr'), all=T)
...
Show that single cell compensation with the correctspillovermeasurement in CellProfiler as well as CATALYST is equivalent
...
datsum = dat %>%
    subset(stack %in% 'FullStack') %>%
    merge(dat_pannel, by='channelnr') %>%
    melt.data.table(id.vars = metavars, variable.name = 'measurement',measure.vars = c('MeanIntensity','MeanIntensityComp','MeanIntensityCompNNLS','MeanIntensityCompFlow', 'MeanIntensityCompLS'))

datsum[, .(mn=mean(value)), by=.(measurement, metal)] %>%
  dcast('metal~measurement')
```
...
```{r}
  ...
  p = dat %>%
    subset(stack %in% 'FullStack') %>%
    merge(dat_pannel, by='channelnr') %>%
    ggplot(aes(x=MeanIntensity, y=MeanIntensityCompNNLS))+
...
print(p)
  ...
  p = dat %>%
    subset(stack %in% 'FullStack') %>%
    merge(dat_pannel, by='channelnr') %>%
    ggplot(aes(x=MeanIntensityComp, y=MeanIntensityCompNNLS))+
    facet_wrap(~metal, scales = 'free')+
    geom_point()+
    geom_abline(slope=1, intercept=0)
    #scale_x_continuous(trans ="asinh2" )+
...
print(p)
  ...
  p = dat %>%
    subset(stack %in% 'FullStack') %>%
    merge(dat_pannel, by='channelnr') %>%
    ggplot(aes(x=MeanIntensityCompFlow, y=MeanIntensityCompLS))+
    facet_wrap(~metal, scales = 'free')+
    geom_point()+
    geom_abline(slope=1, intercept=0)
    #scale_x_continuous(trans ="asinh2" )+
...
```
```{r}
CATALYST::adaptSpillmat(input_sm = as.matrix(sm), out_chs = analysis_channels)
```
